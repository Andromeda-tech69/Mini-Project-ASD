...
def fibonacci_search_title(linked_list, x, descending=False):
    current = linked_list.head
    arr = []

    while current:
        arr.append(current.buku)
        current = current.next
    ...
    while fib > 1:
        ...
        if (not descending and arr[i].judul.lower() < x.lower()) or (descending and arr[i].judul.lower() > x.lower()):
            fib = fib_m_minus_1
            fib_m_minus_1 = fib_m_minus_2
            fib_m_minus_2 = fib - fib_m_minus_1
            offset = i
        elif (not descending and arr[i].judul.lower() > x.lower()) or (descending and arr[i].judul.lower() < x.lower()):
            fib = fib_m_minus_2
    ...
    if fib_m_minus_1 and ((not descending and arr[offset + 1].judul.lower() == x.lower()) or (descending and arr[offset + 1].judul.lower() == x.lower())):
        return offset + 1

    return -1

def fibonacci_search_id(linked_list, x, descending=False):
    current = linked_list.head
    arr = []
    ...
    while fib > 1:
        ...
        if (not descending and arr[i].id < x) or (descending and arr[i].id > x):
            fib = fib_m_minus_1
            fib_m_minus_1 = fib_m_minus_2
            fib_m_minus_2 = fib - fib_m_minus_1
            offset = i
        elif (not descending and arr[i].id > x) or (descending and arr[i].id < x):
            fib = fib_m_minus_2
            fib_m_minus_1 = fib_m_minus_1 - fib_m_minus_2
            fib_m_minus_2 = fib - fib_m_minus_1
        else:
    ...
    if fib_m_minus_1 and ((not descending and arr[offset + 1].id == x) or (descending and arr[offset + 1].id == x)):
        return offset + 1
...
def linked_list_to_list(linked_list):
    ...
    while current:
        arr.tambah_di_akhir(current.buku)
        current = current.next

    return arr

def submenu_lihat_daftar_buku(linked_list):
    while True:
        print("\n\033[1;36m╭──────────────────────────────────────────────────────────────────────╮\033[0m")
        ...
        if pilihan_sub == '1':
            ...
            daftar_buku.tampilkan_semua()
            judul = input("\nMasukkan judul buku: ")
            index = fibonacci_search_title(linked_list, judul)
            if index != -1:
                current = linked_list.head
                for _ in range(index):
                    current = current.next
                current.buku.display_info()
            else:
                print("\nBuku dengan judul '{}' tidak ditemukan.".format(judul))
        elif pilihan_sub == '2':
            daftar_buku.merge_sort(key=lambda x: x.judul.lower(), reverse=True)
            daftar_buku.tampilkan_semua()
            judul = input("\nMasukkan judul buku: ")
            index = fibonacci_search_title(linked_list, judul, descending=True)
            if index != -1:
                current = linked_list.head
                for _ in range(index):
                    current = current.next
            ...
            daftar_buku.tampilkan_semua()
            id_buku = int(input("\nMasukkan ID buku: "))
            index = fibonacci_search_id(linked_list, id_buku)
            if index != -1:
                current = linked_list.head
                for _ in range(index):
                    current = current.next
                ...
                print("\nBuku dengan ID '{}' tidak ditemukan.".format(id))
        elif pilihan_sub == '4':
            daftar_buku.merge_sort(key=lambda x: x.id, reverse=True)
            daftar_buku.tampilkan_semua()
            id_buku = int(input("\nMasukkan ID buku: "))
            index = fibonacci_search_id(linked_list, id_buku, descending=True)
            if index != -1:
                current = linked_list.head
                for _ in range(index):
                    current = current.next
                ...
                print("\nBuku dengan ID '{}' tidak ditemukan.".format(id_buku))
        elif pilihan_sub == '5':
            break
...
def menu_pengguna(daftar_buku):
    while True:
        ...
        if pilihan == "1":
            ...
            print("\033[1;36m Kembali ke Menu Utama... \033[0m")
            break
        else:
            print("\033[91m Pilihan tidak valid. Masukkan angka pilihan yang sesuai. \033[0m")
